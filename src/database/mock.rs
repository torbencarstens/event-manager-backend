...
fn random_ord_range(n: u16, lower: u8, upper: u8) -> String {
    (0..n)
        .map(|_: u16| ((rand::random::<u8>() % (upper - lower)) + lower) as u8)
        .map(|u| char::from(u))
        .collect::<String>()
}

fn random_range(lower: u8, upper: u8) -> u8 {
    ((rand::random::<u8>() % (upper - lower)) + lower)
}

fn random_uppercase(n: u16) -> String {
    random_ord_range(n, 65, 90)
}

fn random_lowercase(n: u16) -> String {
    random_ord_range(n, 97, 122)
}

fn random_website() -> String {
    format!("https://{}.{}",
            random_lowercase(16),
            random_tld())
}
...
fn random_string(n: u16) -> String {
    random_ord_range(n, 33, 126)
}
...
fn random_optional_string(n: u16) -> Option<String> {
    match rand::random::<u8>() % 2 {
